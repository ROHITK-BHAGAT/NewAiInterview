  ...
  const CandidateRow = ({ candidate }) => {
    ...
    return (
      ...
      <div className='col-span-2 flex items-center '>
        <span className='md:text-md text-sm truncate text-wrap md:text-nowrap '>{candidate.candidate_name}</span>
      </div>
      <div className='col-span-2'>
        <p className='text-green-500 font-medium md:text-md text-sm '>{candidate.candidate_resume_selection_status}</p>
      </div>
      <div className='col-span-3'>
        <p className='text-gray-700 md:text-md text-sm truncate'>{candidate.candidate_email}</p>
      </div>
      <div className='col-span-2 text-center'>
        <p className='text-gray-700 md:text-md text-sm'>{candidate.candidate_phone || 'N/A'}</p>
      </div>
      <div className='col-span-1 text-center'>
        <p className='text-blue-500 font-medium md:text-md text-sm'>{candidate.candidate_overall_score || 'N/A'}%</p>
      </div>
      <div className='col-span-2 flex items-center justify-center'>
        <button
          onClick={handleSendEmailClick}
          disabled={sending}
          className={`flex items-center gap-1 md:text-md text-sm cursor-pointer text-blue-500 hover:text-blue-700 ${sending ? 'opacity-70' : ''}`}
        >
...
return (
  <div className='flex flex-col md:flex-row w-full'>
    ...
    <div className='flex-1 flex-col dash'>
      ...
      <div className='flex flex-row justify-between dash1 gap-4'>
        <p className='md:text-3xl text-xl font-bold ml'>Selected Candidates</p>
      ...
      <div className='flex items-center justify-between dash1 searchjob'>
        <div className='flex items-center border border-gray-300 rounded-lg w-full md:w-[50%] h-[50px] p-2 mt-4 md:mt-0'>
      ...
      {/* Status message */}
      {statusMessage && (
        <div className={`text-center py-2 px-4 rounded-md ${statusMessage.includes('successfully') ? 'text-green-500 bg-green-50' : 'text-red-500 bg-red-50'
          }`}>
          {statusMessage}
        </div>
      )}
      ...
      {/* Candidates table */}
      <div className='flex flex-col dash1 overflow-x-auto'>
